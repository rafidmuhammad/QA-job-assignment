...
describe("Integration for delete and get movies should not contain the data added priorly", () => {
    let response;

    const expectedBody = {title : "Life of Rafid", description : "Film tentang filantropi"}
    ...
    test("Then get the movies and validate if the added object priorly is not contained", async() => {
        try {
         response = await axios.get(`http://localhost:5000/allmovie`);
         expect(response.status).toBe(200);
         expect(response.data).toHaveProperty('statusCode',200);
         expect(response.data).toHaveProperty('status','success');
         expect(response.data.data.movies).not.toContainEqual(expectedBody);
